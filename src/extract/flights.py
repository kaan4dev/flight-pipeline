...
def fetch_flights_data(limit=200):
    """
    OpenSky Network API'den gerçek zamanlı uçuş verilerini çeker.
    """
    try:
        ...
        # Kolon adları OpenSky dökümantasyonundan
        columns = [
        ...
        print(f"⚠️ OpenSky API request failed: {e}")
        return pd.DataFrame()


def save_raw_data(df):
    """
    Ham uçuş verisini kaydeder.
    """
...
def main():
    ...
    if not df.empty:
        ...
        print("⚠️ No data fetched (empty response).")
